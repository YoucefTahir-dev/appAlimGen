            ...
            try:
                for language in ('fr', 'ar', 'en'):
                    with self.subTest(language=language):
                        ...
                        page.locator('#id_lines-0-product_search').fill(existing.name[:4])
                        page.locator('.product-result').filter(has_text=existing.name).click()
                        ...
                        modal.locator('[type=submit]').click()
                        expect(modal).not_to_be_visible()
                        expect(row.locator('.product-search')).to_have_value('Sprite ' + language)
                        expect(row.locator('.product-id')).not_to_have_value('')
                        ...
                        page.locator('#add-purchase-line').click()
                        page.locator('#id_lines-2-product_search').fill('Sprite ' + language)
                        page.locator('#id_lines-2-product_results .product-result').filter(has_text='Sprite ' + language).click()
                        expect(page.locator('#id_lines-2-product_search')).to_have_value('Sprite ' + language)
